...
dataMat = x_train(train_labels == Mahalanobis_class);
covMat = cov(dataMat);
invMat = inv(covMat);
transMat = chol(invMat);

x_train = x_train*transMat;
x_test = x_test*transMat;
...
for K = 1:Krange
    tic
    IDX = knnsearch(x_train, x_test,'Distance', 'euclidean','K', K);
    t=toc;
